...
5. Можно творить полную жуть с репозиторием (ну, в Mercurial нельзя - там стоит защита от дурака).
...
2. `git config --global core.editor notepad` (или Nano, или Vim, если не устраивает вариант по умолчанию - Vim).
...
4. `git status` (очень полезная штука в любой непонятной ситуации!), смотрим на "Untracked files" - найдены, но не в репозитории. Делаем `git add` (но без теста), теперь staged - готовы к коммиту. Только staged пойдёт в следующий коммит!
5. `git commit`, вводим сообщение. Также можно `git commit -m"foobar"` и `git commit -Ffile.txt`.
6. Делаем `git clone` из другой папки. Упс, забыли файл, надо добавить. Добавляем в старой папке, коммит с комментарием "забыли файл там-то".
...
4. Начали добавлять функциональность "товар по цене и в количестве", но посередине вспомнили про тесты, `git stash`, `git stash list`, `git show stash@{0}` или `git stash show -p`. Намекнуть про интерактивный stash (можно спрятать только кусочек и там много возможностей), что прячутся именно отличия (а не файлы целиком), что они представляют собой стэк из нескольких независимых.
...
4. Теперь переключимся обратно в master и там срочный фикс: добавить подсказки пользователю (что вводить). Обновили README (добавив опечатку), закоммитили. Посмотрели историю при помощи `git log --all --graph --online --decorate`, вау (или загуглить: git beautiful log).
5. Переключаемся в add-barcodes и дописываем считывание из файла. Коммитим. Замечаем, что подсказок всё ещё нет.
6. Переключаемся в master, делаем merge, огребаем конфликт в считывании (привет, git status). Ну, он простой, разрешили (показать, какие где файлы, намекнуть про take theirs/take ours). Закоммитили. Намекнуть про `--abort`, и пометки разрешённого файла. А вот в тестах
...
5. Что делать:
	...
	2. Обычно работает `git pull --rebase`. Вытягивает изменения и пытается применить наши изменения к удалённой ветке. ПЕРЕПИСЫВАЕТ ИСТОРИЮ!!1! Если получилось - успех. Если не получилось - merge conflict, править руками, можно в любой момент сделать `git merge --abort`.
	3. Намёк: можно создать точку в старом месте, `git fetch` (только поменяет `origin/master`), перенести ветку на `origin/master` и сделать то же самое руками. Или при помощи `merge`. На самом деле pull - это fetch + merge.
...
На http://learngitbranching.js.org/ это remote1-remote7.
...
Намёк: кое-где вам расскажут про `--amend` (например, если опечатались в сообщении коммита), этот ключик не создаёт новый коммит, а меняет последний. Так вот, ПЕРЕПИСЫВАЕТ ИСТОРИЮ!!1! Если уже запушили с опечаткой - очень жаль.
Будут ещё рассказывать про `--force` - бегите. Ну, впрочем, после `pull --rebase` ничего другого и не остаётся.

Самая популярная проблема: сделали коммит, не сделали push.

Стиль сообщений коммитов
------------------------

См. http://chris.beams.io/posts/git-commit/

Замечания про стиль сообщений коммитов (чтобы было остальным удобно читать): отдельной строчкой информацию (как можно короче, по грамматике - this commit "Does this and that"), потом в самом коммите - описание, зачем, почему (а не "как" - для этого есть тело), ссылки на баги и так далее. C заглавной буквы, без точки в конце. Ещё полезно ограничивать длину, потому что в консоли обрежется.

Идеально: одно действие - один коммит. Чтобы можно было просто откатить в случае чего. И легко проверить корректность изменений. По факту могут склеить несколько простых изменений в один коммит, но, скажем, в Google так делать запрещают и просто не дадут закоммитить такое изменение "одним махом".

Pull requests
-------------

Инструмент, который есть на GitHub (да и на других хостингах тоже). Идея: предложить изменения в какой-то репозиторий, к которому нет доступа.
Шаги:
1. Выбираем репозиторий, в который хотим внести изменения. Например, https://github.com/yeputons/pull-request-demo.
2. Делаем fork на GitHub. Смысл такой: надо, чтобы на GitHub как-то попали наши изменения. В основной репозиторий мы не можем, а вот создать себе копию репозитория - можем. Fork делает это.
3. Клонируем репозиторий себе локально.
4. Вносим изменения, делаем push. Надо добавить себя в список if'ов. Сначала добавляйте имена с багами, я буду принимать.
5. Находим кнопку "Create pull request" на странице своего репозитория, нажимаем, оформляем pull request в основной репозиторий.
6. Я смотрю на код (и отдельные коммиты, и весь патч), пишу замечания, комментирую, в какой-то момент принимаю ваши изменения.
7. Возможно, вам придётся переделать коммит, чтобы не было конфликтов (обычно тут хорошо работает `git pull --rebase`). Потребуется `git remote add upstream`, `git fetch upstream` и `git merge upstream/master` (не забыть потом запушить в origin, upstream только на чтение работает; upstream - просто кодовое название). Тут-то и потребуются те, кто закоммитил с багами. Вам тоже надо поправить.
8. Я нажимаю "merge", изменения попадают в мой репозиторий.

Также можно использовать pull requests из своего репозитория в свой же, чтобы показать какой-то код сокоманднику. А дальше либо нажать "merge" в интерфейсе (squash тоже можно), либо руками всё сделать и закрыть Pull Request.

Что дальше
==========

Пробуйте использовать систему контроля версий в тех местах, где у вас код живёт сильно дольше одного дня.
Если вы чувствуете, что она вам скорее мешает, чем помогает - гуглите и спрашивайте товарищей, как использовать эффективнее. Система должна помогать работать, а не мешать.
В идеале система контроля версий вам вообще ни разу не потребуется, потому что вы пишете код без багов и всё идеально помните. Так что если только делаете коммиты, но не смотрите - в целом это нормально и так бывает.

Можно пройти игру: https://github.com/git-game/git-game . Там будут ещё полезные команды: например, двоичный поиск места, где кто-то что-то сломал в репозитории или `git blame`, которая показывает, кто когда менял последним какую строчку файла. Бывает полезно даже самому себе: если увидел фигню в файле, то стоит проверить весь коммит - вдруг там ещё какая-то фигня затесалась?

Проверяйте перед коммитом при помощи `git diff --cached`.
Не добавляйте при помощи `git add .`.
